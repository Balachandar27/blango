...
class PostApiTestCase(TestCase):
  def setUp(self):
    ...
    self.posts = [
      Post.objects.create(author=self.u1, published_at=timezone.now(), title="Post 1 Title", slug="post-1-slug", summary="Post 1 Summary", content="Post 1 Content",),
      Post.objects.create(author=self.u2, published_at=timezone.now(), title="Post 2 Title", slug="post-2-slug", summary="Post 2 Summary", content="Post 2 Content",),
    ] 

    self.post_lookup = {p.id: p for p in self.posts}
  ...
  def test_unauthenticated_post_create(self):
    self.client.credentials() # this will logout prior to the testcase run
    post_dict = {
    "title": "Test Post",
    "slug": "test-post-3",
